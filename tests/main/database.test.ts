...
describe("database", () => {
  ...
  it("migrates UTC timestamps and audio paths to local time once", () => {
    ...
    const oldPath = "2026-04-17T00-00-00";

    // In UTC the local-time path equals the old path, so no filesystem
    // rename is needed and the migration returns an empty array. In any
    // other TZ the migration records the single rename.
    if (expectedPath === oldPath) {
      expect(renames).toEqual([]);
    } else {
      expect(renames).toEqual([{ id: 1, oldPath, newPath: expectedPath }]);
    }
  ...
  it("rejects invalid download update columns", () => {
    const id = createDownload(db, {
      url: "https://example.com",
      source: "example.com",
    });

    expect(() => updateDownload(db, id, { bogus: "value" } as any)).toThrow(
      "Invalid column name: bogus",
    );
  });
});
