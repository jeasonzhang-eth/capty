...
- **会话管理** — 每次录音自动创建会话，历史列表按分类文件夹 + 时间两级分组（4 个分类：下载内容 / 个人录音 / 会议 / 电话，每个分类内按 Today / Yesterday / Previous 7 Days / Previous 30 Days / Older 分组），支持折叠/展开；右键重命名（行内编辑，同步重命名磁盘音频目录和主音频文件）/ 删除（含确认弹窗，同时清理音频文件）/ 移动到其他分类（"Move to..." 子菜单）；AI 一键重命名（✨ 按钮，使用 Rapid Model LLM 根据转录内容自动生成标题）；标题支持两行显示，适配长标题（时间戳+标题格式）；下载内容自动归入"下载内容"分类，录音默认归入"个人录音"
