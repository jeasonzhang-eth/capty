...
- **会话管理** — 每次录音自动创建会话，历史列表按分类文件夹 + 时间两级分组（4 个分类：下载内容 / 个人录音 / 会议 / 电话，每个分类内按 Today / Yesterday / Previous 7 Days / Previous 30 Days / Older 分组），支持折叠/展开；右键重命名（行内编辑，同步重命名磁盘音频目录和主音频文件）/ 删除（含确认弹窗，同时清理音频文件）/ 移动到其他分类（"Move to..." 子菜单）；AI 一键重命名（✨ 按钮，使用 Rapid Model LLM 根据转录内容自动生成标题）；标题支持两行显示，适配长标题（时间戳+标题格式）；下载内容自动归入"下载内容"分类，录音默认归入"个人录音"；**拖拽排序** — 支持 HTML5 原生拖拽：跨分类拖拽 session 到目标分类头部或空分类区域即可移动分类；同分类内拖拽调整排序（drop indicator 横线指示插入位置），拖动时行半透明 + 目标分类高亮虚线框
...
- **Voice 列表接口简化** — `tts:list-voices` 不再需要任何参数，sidecar `/v1/audio/voices` 直接从已加载模型读取 voice 列表，模型未加载时自动扫描 TTS 模型目录的 config.json 获取 voice
...
- **会话日期显示星期** — 历史列表中每个 session 的日期末尾新增星期几显示（如 "Mar 30, Sun, 10:42"）

### 2026-03-29 (58)

- **引入 Lucide 图标库** — 播放栏所有图标替换为 [lucide-react](https://lucide.dev/) 组件：`RotateCcw`/`RotateCw`（内嵌 "15" 标注）用于快退/快进、`Play`/`Pause` 用于播放控制、`X` 用于关闭按钮；告别手写 Unicode/SVG，图标风格统一专业
- **双击会话播放/停止** — 在历史列表双击已完成的会话开始播放，正在播放时再次双击停止播放；录制中禁用双击播放
- **播放栏双行布局重设计** — PlaybackBar 改为上下两行布局：上行为 当前时间 | 波形进度条 | 总时长 | 倍速按钮 | 关闭按钮；下行为居中的 后退15s | 播放/暂停（圆形 accent 按钮）| 前进15s；移除文件名显示；跳转时间从 10s 改为 15s；播放控制按钮使用 SVG 图标（带 "15" 标注的回退/前进箭头）
- **播放栏 Crossfade 过渡** — 底部录制按钮区域与播放栏平滑切换：录制按钮 0.3s 淡出下移，播放栏 0.3s 淡入上浮；两者共享同一 100px 高度容器，底部区域高度始终固定无跳动
...
- **记忆播放进度** — 每个会话自动保存音频播放位置，暂停/停止/切换会话时自动记录进度，下次播放同一会话时从上次位置恢复；播放完毕自动重置进度为 0；sessions 表新增 `playback_position` 列

### 2026-03-29 (56)

- **SummaryCard 导出菜单** — SummaryPanel 每个 LLM 生成内容卡片底部新增 ⬇ 导出按钮，点击弹出下拉菜单支持 6 种导出方式：
  - **复制为纯文本** — 去除 Markdown 格式和 `<think>` 标签，复制到剪贴板
  - **复制为图片** — 使用 `html-to-image` 将渲染后的 Markdown 内容转为 PNG 并复制到剪贴板
  - **导出为图片** — 生成 PNG 并通过系统保存对话框保存到本地
  - **导出为 Markdown** — 保存 `.md` 文件，自动去除 `<think>` 思考标签
  - **导出为 Markdown（含思考）** — 保存原始 `.md` 内容（仅在内容包含 `<think>` 标签时显示此选项）
  - **导出为 Word** — 生成 Office 兼容 HTML 保存为 `.doc` 文件
  - 新增 `export:save-buffer` IPC handler 支持二进制文件保存（图片/Word）
  - 新增 `html-to-image` 依赖

### 2026-03-29 (55)

- **播放+转录字幕滚动优化** — 修复播放和转录同时进行时字幕在"中间"和"底部"之间跳来跳去的问题：内部检测 segments 是否正在增长（转录进行中），增长期间及停止后 3 秒内自动切换为手动滚动模式（保持底部跟随新段落），避免 react-lrc 的居中滚动冲突；同时保留播放行高亮（橙色左边框+背景）和点击跳转；转录完成 3 秒后自动恢复 react-lrc 歌词同步模式

### 2026-03-28 (54)

- **AI Rename 会话标题** — 左侧历史列表每个已完成会话支持 AI 重命名：标题行右侧 hover 显示 ✨ 按钮，右键菜单也有 "AI Rename" 选项（含 sparkle 图标），点击后使用 Rapid Model（未设置时自动 fallback 到第一个可用 LLM Provider）根据转录内容自动生成简洁标题（非流式调用，最多 10 词），自动拼接时间戳前缀（格式：`2026-03-25T10-42-06：标题`）；生成中显示旋转加载动画；Settings → Default Models → Rapid Model 卡片内新增 "Rename Prompt" 文本框，可自定义 AI 重命名提示词，重启后保留；无任何已配置 LLM Provider 时按钮和菜单项不显示
...
- **修复模型大小显示不准确** — 推荐模型和已下载模型的大小均可能显示错误：
  - 已下载模型：`readModelMeta` 从 `model-meta.json` 读取下载时写入的旧 `size_gb`（来自错误的硬编码值），改为始终用 `calcDirSizeGb()` 计算实际磁盘大小
  - 未下载推荐模型：更新 `recommended-models.json` 硬编码值（如 0.6B-4bit 从 0.25→0.70 GB），并从 HuggingFace tree API 获取精确大小
  - `models:list` 和 `tts-models:list` 改为异步以支持 API 查询
...
- **不兼容模型类型检测** — 从 HuggingFace 下载的部分模型（如 `funasr` 类型）不被 mlx-audio STT 支持，加载时 500 错误：
  - 新增 `isModelSttSupported()` 检查：读取模型 `config.json` 的 `model_type`，与 `KNOWN_STT_TYPES`（mlx-audio MODEL_REMAPPING keys + values）比对
  - `KNOWN_STT_TYPES` / `KNOWN_TTS_TYPES` 同时包含 remapping keys 和 module values（如 `glm` + `glmasr`、`vibevoice` + `vibevoice_asr`），确保 config.json 中无论写 key 还是 module name 都能正确识别
