...
- **歌词式字幕同步** — 基于 react-lrc 的专业字幕同步，纯回放时自动高亮当前段落并居中显示（首尾行也能居中）；用户手动滚动后 5 秒自动恢复跟随；点击任意段落跳转到对应音频位置；录音+播放同时进行时自动切换为手动滚动模式（保持底部跟随新段落），避免与 react-lrc 的居中滚动冲突，同时保留播放行高亮
...
- **录音+播放字幕滚动优化** — 修复同时录音和播放时字幕跳来跳去的问题：录音期间不再使用 react-lrc 自动居中滚动（会与新段落到达的底部滚动冲突），改为使用手动滚动模式保持在底部，同时保留当前播放行高亮（橙色左边框+背景）和点击跳转；仅在纯回放模式（非录音时）才使用 react-lrc 歌词同步
